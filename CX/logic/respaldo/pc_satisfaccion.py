...
def cargaRespuestasPCS(db, Year,Trimestre, results, found_list):
    ...
    try:
        ...
        flash("Error al cargar la información de Satisfacción", "error")
...
def chart_pcs():
    ...
    error = "Error al cargar reporte PCS"
    ...
    if request.method == 'GET':
        try:        
            #Conexion con la DB - KPI's PCS
            kpi_clients = db.collection('PCS_KPIS').order_by("Cliente").get() #Get All PCS KPI's #2022, 2021
